	...
	if (rbl->options & MODE_LONGFN)
	{
		if (rext.data->fnsize < 30 || rext.data->fnsize > 108)
		{
			...
			else
			{
				...
				{
					fixederror("Teset filename size to 30");
					rext.data->fnsize = 30;
				}
				else
				{
					fixederror("Teset filename size to 100");
...
static error_t RepairBootBlock(void)
{
	...
	// read bootblock 
	bootbl.data = pfscalloc(1, MAXRESBLOCKSIZE);
...
static bool dd_CheckBlock(uint32 bloknr, int seqnr)
{
	...
	/* get block */
	ddblk.data = pfscalloc(1, SIZEOF_RESBLOCK);
...
static void fixedfileerror(char *errortxt,...)
{
	...
	va_start (parms, errortxt);
	vsprintf(txt, errortxt, parms);
...
static error_t RepairDirBlock(uint32 bloknr, uint32 anodenr, uint32 parent)
{
	...
	/* get block */
	dirblk.data = pfscalloc(1, SIZEOF_RESBLOCK);
...
static error_t RepairFile(struct direntry *de, c_dirblock_t *parent)
{
	...
	if (ss.stage == syntax)
	{
		...
		{
			addfileerror("Invalid filesize %08x-%08x %ld", high, size, blokken);
			return e_remove;
		}
	}
...
error_t RepairAnodeBlock(uint32 *bloknr, uint32 seqnr)
{
	...
	// get anodeblock
	ablk.data = pfscalloc(1, SIZEOF_RESBLOCK);
	if (*bloknr)
	{
...
error_t RepairBitmapBlock(uint32 *bloknr, uint32 seqnr)
{
	...
	// get anodeblock
	bmblk.data = pfscalloc(1, SIZEOF_RESBLOCK);
	...
	uint32 *bloknr, uint32 seqnr)
{
	...
	if (!iblk.data)
	{
		iblk.data = pfscalloc(1, SIZEOF_RESBLOCK);
...
static error_t InitReservedBitmap(void)
{
	...
	// check cluster size which is the rootblock together with the reserved bitmap.
	// There is one bit per reserved block in the reserved bitmap
...
static error_t RepairMainBitmap(void)
{
	...
	bmb.data = pfscalloc(1, SIZEOF_RESBLOCK);
...
static bitmap_t *bg_InitBitmap(uint32 start, uint32 stop, uint32 step)
{
	...
	if (!(mybitmap = pfscalloc(1, sizeof(bitmap_t))))
		return NULL;
	...
	if (!(mybitmap->map = pfsmalloc(4 * mybitmap->lwsize)))
	{
		pfsfree(mybitmap);
